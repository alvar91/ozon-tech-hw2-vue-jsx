      ...
      <aside class={$style.aside}>
        <form action="#" class={$style.aside__form}>
          <fieldset class={$style.aside__fieldset}>
            <button class={$style.aside__search} type="submit" aria-label="Поиск по сайту"></button>
            <input
          ...
          <button class={$style.aside__filter} type="button" aria-label="Фильтр поиска"></button>
        </form>

        <AsideList
          users={users}
          on={{
            setCurrentUser: (evt) => setCurrentUser(evt),
          }}
        />
      </aside>
    );
